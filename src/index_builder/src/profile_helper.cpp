...
#define ZKGROUP_FLAG_STR         "F_ZKGROUP"
 ...
 */
int load_conf(ib_ctx_t *pctx) {
    char  command[PATH_MAX];
    FILE *conf_fd = NULL;
    int   ret     = 0;
    mxml_node_t *tree = NULL;
    conf_fd = fopen(pctx->conf_file, "r");
    do {
        if (conf_fd == NULL) {
...
/*        mxml_node_t *global_node  = NULL;
        ...
        const char *dict_path = "dict";
        snprintf(root_path,sizeof(root_path),"%s/",pctx->output_dir);
        snprintf(full_path, PATH_MAX, "%s/%s", root_path, dict_path);
        snprintf(command, 1024, "rm -rf %s; mkdir -p %s", full_path, full_path);
        fprintf(stderr,"PROFILE: root_path:%s\n",root_path);
        fprintf(stderr,"PROFILE: full_path:%s\n",full_path);
...
//        char *profile_path = (char *)mxmlElementGetAttr(profile_node, "sub_dir");
        snprintf(full_path, PATH_MAX, "%s/profile/", root_path);
        snprintf(command,   1024, "rm -rf %s; mkdir -p %s", full_path, full_path);
...
//            fprintf(stderr,"MARK!!!! line:%d\n",__LINE__);
                    ...
                    bit_field_node != NULL; bit_field_node = mxmlFindElement(bit_field_node, tree, "field", NULL, NULL, MXML_NO_DESCEND))
            {
                ...
                if (field_type == DT_UNSUPPORT || field_type == DT_KV32) {
                    ret = -1;
                    fprintf(stderr,"RETURNED! line:%d\n",__LINE__);
                    break;
                }
                ...
                else if (strcmp(fieldFlag_str, ZKGROUP_FLAG_STR) == 0) {
                    bitParam.fieldFlagArr[bitParam.field_num] = F_ZK_GROUP;
                }
                else {
                    bitParam.fieldFlagArr[bitParam.field_num] = F_NO_FLAG;
                }
            ...
            ret = pManager->addBitRecordField(bitParam.field_num, bitParam.fieldNameArr, bitParam.fieldAliasArr,
                                              bitParam.bitLenArr, bitParam.typeArr, bitParam.encodeFlagArr,
                                              bitParam.fieldFlagArr, bitParam.defaultArr, groupPos);
...
//int do_build_profile(const char *conf_file, const char *head_file, const char* data_file)
int do_build_profile(ib_ctx_t *pctx)
{
    const char *conf_file = pctx->conf_file;
    char head_file[PATH_MAX];
    char data_file[PATH_MAX];
    snprintf(head_file,sizeof(head_file),"%s/profile.mdl",pctx->output_dir);
    ...
    do
    {
        ...
        if (fgets(data_str, MAX_DOC_LEN * sizeof(char), header_fd) == NULL) {
        ...
        for(i = 0; i<pctx->thread_count;i++){
            snprintf(data_file,sizeof(data_file),"zcat %s/"PROFILE_FILENAME".gz",pctx->tmp_dir,i);
            TLOG("INDEXLIB: ProfileBuilder proccessing %s/"PROFILE_FILENAME".gz",pctx->tmp_dir,i);
            data_fp = popen(data_file,"r");
            //fprintf(stderr,"popen fp = %p\n",data_fp);
            if(!data_fp){
                TERR("INDEXLIB: Can not open data_file for read. %s\n",data_file);
                return -1;
            }
            snprintf(data_str,MAX_DOC_LEN,"%u\001",vdocid);
            pre_len = strlen(data_str);
            while(fgets(data_str+pre_len, MAX_DOC_LEN-pre_len,data_fp) != NULL) {
                process_str(data_str);
                ...
                vdocid++;
                snprintf(data_str,PATH_MAX,"%u\001",vdocid); 
                pre_len = strlen(data_str);
            }
            pclose(data_fp);
        }
